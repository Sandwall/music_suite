...
namespace gfx {
	...
	void Renderer::add_rect(f32 x, f32 y, f32 w, f32 h, i32 texIdx, const Color& color) {
	...
	void Renderer::add_rect(f32 x, f32 y, f32 w, f32 h, const Color& color) {
		...
		add_rect(x, y, w, h, 0, color);
	}

	void Renderer::add_char(f32 x, f32 y, f32 w, f32 h, i32 fontId, i32 codepoint, const Color& color) {
		...
		Rect uvRect = { 0.0f, 0.0f, 1.0f, 1.0f };
		if (fontAtlasRef) {
			stbtt_packedchar packedChar = fontAtlasRef->packedChars.get(codepoint, fontId);
			uvRect.x = static_cast<f32>(packedChar.x0) / static_cast<f32>(fontAtlasRef->bitmap.width);
			uvRect.y = static_cast<f32>(packedChar.y0) / static_cast<f32>(fontAtlasRef->bitmap.height);
			uvRect.w = static_cast<f32>(packedChar.x1 - packedChar.x0) / static_cast<f32>(fontAtlasRef->bitmap.width);
			uvRect.h = static_cast<f32>(packedChar.y1 - packedChar.y0) / static_cast<f32>(fontAtlasRef->bitmap.height);
		}
